...
class Snatch3rRobot(object):
    ...
    def go(self,
           left_wheel_duty_cycle_percent: object = 100,
           right_wheel_duty_cycle_percent: object = 100) -> object:
    ...
    def turn(self,duty_cycle_percent,time,direction):
        time0=t.time()
        if direction == 'right':
            self.left_wheel.start_spinning(duty_cycle_percent)
            while True:
                if (t.time()-time0) >= time:
                    break
